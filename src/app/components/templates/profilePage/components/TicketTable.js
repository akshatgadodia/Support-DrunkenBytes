...
const TicketTable = (props) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const ticketsData = await sendRequest(
        `/ticket/get-all-tickets?filters=${filterParams}&sort=${JSON.stringify(
          sort
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "createdBy",
      key: "createdBy",
      render: (_, { createdBy, name }) => (
        createdBy?.name !== undefined ? <Link href={`/users/${createdBy._id}`}>{createdBy.name}</Link> : <div>{name}</div>
      ),
    },
    {
      title: "Subject",
